...
def test_create_cube_transit_network_from_file(request):
    print("\n--Starting:", request.node.name)
    from lasso import CubeTransit
    cube_transit_net = CubeTransit.create_cubetransit(cube_transit_file=os.path.join(CUBE_DIR,"single_transit_route_attribute_change","transit.LIN"))
    ## todo write an assert that actually tests something
...
def test_write_cube_transit_standard(request):
    ...
    cube_transit_net = CubeTransit.create_cubetransit(
        cube_transit_file=os.path.join(CUBE_DIR,"single_transit_route_attribute_change","transit.LIN")
    )
