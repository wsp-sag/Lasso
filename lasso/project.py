...
class Project(object):
    ...
    def add_highway_changes(self):
        ...
        base_nodes_df.rename(columns={"x": "X", "y": "Y"}, inplace=True)
        ...
        link_changes_df.rename(
            columns={
                "ISDRIVELIN": "isDriveLink",
        ...
        node_changes_df.rename(
            columns={
                "ISDRIVENOD": "isDriveNode",
                "N": "travelModelId",
        ...
        def consolidate_actions(log, base, key_list):
            ...
            log_df["OPERATION_final"] = log_df.apply(lambda x: final_op(x), axis=1)
            return log_df[changeable_col + ["OPERATION_final"]]

        if len(link_changes_df) != 0:
            link_changes_df = consolidate_actions(
                link_changes_df, base_links_df, ["A", "B"]
            )

        if len(node_changes_df) != 0:
            node_changes_df = consolidate_actions(
                node_changes_df, base_nodes_df, ["travelModelId"]
            )
        ...
        try:
            ...
            add_link_dict_df["properties"] = add_link_dict_df.apply(prop_dict, axis=1)
        ...
        try:
            changeable_col = [
                x for x in link_changes_df.columns if x in base_links_df.columns
            ]
            ...
            for i in cube_change_df.index:
                ...
                for x in changeable_col:
                    if (change_df[x] == base_df[x]) | (
                        x in ["AREA", "COUNTY", "ASGNGRP", "CENTROID"]
                    ):
                        continue
                    else:
                        out_col.append(x)
            ...
            change_link_dict_df = (
                change_link_dict_df.groupby("properties")[["A", "B", "LINK_ID"]]
                .agg(lambda x: list(x))
                .reset_index()
            )
        ...
        card_dict = {
            "project": "TO DO User Define",
            "changes": list(
                filter(
                    None, [delete_link_dict] + [add_link_dict] + change_link_dict_list
                )
            ),
        }
